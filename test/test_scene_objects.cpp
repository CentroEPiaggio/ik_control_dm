...
{
    std::cout<<std::endl;
    std::cout<<"|Dual manipulation| -> ik_control : test_scene_objects "<<std::endl;
    std::cout<<std::endl;

    ros::init(argc, argv, "test_scene_objects");
    ...
    {
	ROS_INFO("IK_control:test_scene_objects : %s object request accepted: %d", srv_obj.request.command.c_str(), (int)srv_obj.response.ack);
    }
    else
    {
